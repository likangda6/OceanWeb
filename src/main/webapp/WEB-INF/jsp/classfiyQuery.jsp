...
<div class="body">
    <div class="navbar">
        ...
        <div class="left-nav" style="margin-left: 50px;">
            <div class="dropdown " onclick="window.location.href='/index'">
                <button class="dropbtn">首页</button>
            </div>
            <div class="dropdown changed">
                <button class="dropbtn" style="color: yellow;">档案查询</button>
            </div>
    ...
    <div class="search-container fade-in">
        <div class="classification-window">
            <div class="classification-select" id="class-select">
                <div class="classification-header">
                    <span class="selected-value">选择纲</span>
                    <span class="arrow">▼</span>
                </div>
                <div class="classification-options" id="class-options"></div>
            </div>

            <div class="divider"></div>

            <div class="classification-select" id="order-select">
                <div class="classification-header">
                    <span class="selected-value">选择目</span>
                    <span class="arrow">▼</span>
                </div>
                <div class="classification-options" id="order-options"></div>
            </div>

            <div class="divider"></div>

            <div class="classification-select" id="family-select">
                <div class="classification-header">
                    <span class="selected-value">选择科</span>
                    <span class="arrow">▼</span>
                </div>
                <div class="classification-options" id="family-options"></div>
            </div>
    ...
    <div class="results-container" id="results-container" style="display: none;">
        <div class="results-title" id="results-title">查询结果</div>
    ...
    <div class="modal" id="details-modal">
        <div class="modal-content">
            ...
            <div class="modal-title" id="modal-title">物种详情</div>
...
<script>
    ...
    function initClassSelect() {
        ...
        Object.keys(staticData).forEach(className => {
            const option = createOption(className, () => {
                updateSelected(classSelect, className);
                loadOrders(className, staticData);
                resetSelect(familySelect, '选择科');
            });
            classOptions.appendChild(option);
        });
    }

    function loadOrders(className, data) {
        orderOptions.innerHTML = '';
        const orders = data[className];
        if (!orders) {
            resetSelect(orderSelect, '选择目');
            resetSelect(familySelect, '选择科');
            return;
        }
        Object.keys(orders).forEach(orderName => {
            const option = createOption(orderName, () => {
                updateSelected(orderSelect, orderName);
                loadFamilies(className, orderName, data);
            });
            orderOptions.appendChild(option);
        });
    }

    function loadFamilies(className, orderName, data) {
        familyOptions.innerHTML = '';
        const families = data[className]?.[orderName];
        if (!families) {
            resetSelect(familySelect, '选择科');
            return;
        }
        families.forEach(familyName => {
            const option = createOption(familyName, () => {
                updateSelected(familySelect, familyName);
            });
            familyOptions.appendChild(option);
        });
    }

    function createOption(text, clickHandler) {
    ...
    function updateSelected(selectElement, value) {
        selectElement.querySelector('.selected-value').textContent = value;
        // 移除其他选项的高亮
        const options = selectElement.querySelectorAll('.classification-option');
        options.forEach(opt => opt.classList.remove('selected'));
        // 高亮当前选项
        const currentOption = Array.from(options).find(opt => opt.textContent === value);
        if (currentOption) currentOption.classList.add('selected');
    }

    function resetSelect(selectElement, placeholder) {
        selectElement.querySelector('.selected-value').textContent = placeholder;
        selectElement.querySelector('.classification-options').innerHTML = '';
    }
    ...
    function setupMobileDropdowns() {
        ...
        selects.forEach(select => {
            const header = select.querySelector('.classification-header');
    ...
    queryButton.addEventListener('click', () => {
        const selectedClass = classSelect.querySelector('.selected-value').textContent;
        const selectedOrder = orderSelect.querySelector('.selected-value').textContent;
        const selectedFamily = familySelect.querySelector('.selected-value').textContent;

        if (selectedClass === '选择纲') {
            showAlert('请选择纲');
            return;
        }
        if (selectedOrder === '选择目') {
            showAlert('请选择目');
            return;
        }
        if (selectedFamily === '选择科') {
            showAlert('请选择科');
            return;
        }
    ...
    function displayResults(data) {
        resultsTitle.textContent = `查询结果 (共 ${data.length} 个物种)`;
        resultsBody.innerHTML = '';

        if (data.length === 0) {
            const row = document.createElement('tr');
            row.innerHTML = `<td colspan="6" style="text-align: center; padding: 20px; color: #666;">没有找到匹配的物种</td>`;
    ...
    function createAnimalRow(animal) {
        ...
        row.innerHTML = `
            ...
            <td>${animal.order}</td>
            <td>${animal.family}</td>
            <td><img src="${animal.imageUrl || 'https://via.placeholder.com/80x60?text=No+Image'}" alt="${animal.name}" class="animal-image"></td>
            <td><span class="view-details" data-id="${animal.id}">查看详情</span></td>
        `;
        const detailsButton = row.querySelector('.view-details');
        detailsButton.addEventListener('click', () => showDetails(animal));
        return row;
    }
    ...
    function showDetails(animal) {
        ...
        modalBody.innerHTML = `
            <p><strong>纲:</strong> ${animal.animalClass}</p>
            <p><strong>目:</strong> ${animal.order}</p>
            <p><strong>科:</strong> ${animal.family}</p>
            <div style="text-align: center; margin: 15px 0;">
                <img src="${animal.imageUrl || 'https://via.placeholder.com/300x200?text=No+Image'}" alt="${animal.name}" style="max-width: 100%; max-height: 300px; border-radius: 8px;">
            </div>
            <p style="white-space: pre-line;"><strong>描述:</strong> ${animal.description || '暂无详细描述'}</p>
        `;
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // 窗口大小变化时重新检测是否需要绑定移动端逻辑（已通过isMobileDevice动态判断）
    });
</script>
</body>
</html>