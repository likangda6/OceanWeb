...
<head>
  ...
  <style>
    ...
    /*  background-repeat: no-repeat;*/
    /*  background-image: url("/images/ai/aa.png");*/
    /*  color: white;*/
    /*  border-radius: 50%;*/
    /*  display: flex;*/
    /*  justify-content: center;*/
    ...
    .hot-questions {
      width: 80%;
      margin-top: 20px;
      background-color: rgba(224, 247, 255, 0.5); /* 浅蓝色背景 */
      border-radius: 12px;
      padding: 10px;
      margin-bottom: 10px;
    ...
    .question-tags {
      ...
      flex-wrap: wrap;
      gap: 10px;
      justify-content: center; /* 使标签居中对齐 */
      margin-top: 10px;
    }
    ...
    .refresh-btn {
      ...
      display: block;
      position: relative;
      left: calc(100% - 90px); /* 根据需要调整位置，让其靠右显示 */
      width: fit-content;
      padding: 5px 10px;
    ...
    .content1 {
      width: 90%;
      height: calc(100% - 50px); /* Subtracting the title height */
      margin: 0 auto;
      padding: 10px;
      display: flex;
      flex-direction: column;
    ...
    <div class="chat-window">
      <div class="box">
        <div class="chat-content" ref="chatContent">
          ...
          <div class="ai-message" v-for="(msg, i) in messages" :key="i">
            <div class="ai-text" v-if="msg.role === 'ai'" v-html="msg.html || msg.content"></div>
            <div class="user-text" v-else>{{ msg.content }}</div>
          </div>
        ...
        <div class="input-area">
          ...
          <div class="input-container">
            ...
            <div class="send-btn" :class="{ active: userInput.trim() }" @click="sendMessage"></div>
          </div>
          <div class="input-hint">shift+enter换行，enter发送</div>
        </div>
      </div>
    </div>
    <div class="notice">
  ...
  new Vue({
    ...
    methods: {
      ...
      showStaticResponse: function (response) {
        ...
        function typeWriter() {
          if (index < response.length) {
            ...
            if (self.ttsEnabled && !self.isSpeaking) self.speakWithChildVoice(response);
          }
        }
        typeWriter();
      },
      callStreamingAPI: function (question) {
        ...
        var fullResponse = '';
        var eventSource = new EventSource('/ai/chatStream?question=' + encodeURIComponent(question));
        ...
        eventSource.onerror = function () {
          ...
          if (self.ttsEnabled && fullResponse && !self.isSpeaking) self.speakWithChildVoice(fullResponse);
        };
      },
      // === 语音合成（TTS） ===
      ...
      speakWithChildVoice: function (text) {
        ...
        try {
          ...
          utterance.onstart = function () { if (self.spriteEl) self.spriteEl.style.animationPlayState = 'running'; };
