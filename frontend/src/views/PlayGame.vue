<template>
  <div class="game-page">
    ...
    <!-- 背景音乐 -->
    <audio ref="bgMusic" id="bgMusic" loop preload="none">
      <source src="/images/game/RiverR.mp3" type="audio/mpeg">
    </audio>
    <!-- 音效 -->
    <audio ref="correctSound" id="correctSound" preload="auto">
      <source src="/images/game/success.wav" type="audio/mpeg">
    </audio>
    <audio ref="wrongSound" id="wrongSound" preload="auto">
...
export default {
  ...
  computed: {
    bgMusic() { return this.$refs.bgMusic; },
    correctSound() { return this.$refs.correctSound; },
    wrongSound() { return this.$refs.wrongSound; }
  },
  mounted() {
    this.initSettings();
  ...
  methods: {
    ...
    initSettings() {
      ...
      if (this.bgMusic) this.bgMusic.volume = this.volumeLevel / 100;
      if (this.musicEnabled) {
        if (this.bgMusic) this.bgMusic.play().catch(function (e) { console.log('自动播放被阻止:', e); });
      } else {
    ...
    toggleMusic(val) {
      ...
      if (val) {
        if (this.bgMusic) this.bgMusic.play().catch(function (e) { console.log('播放被阻止:', e); });
      } else {
        if (this.bgMusic) this.bgMusic.pause();
      }
    },
    ...
    showModal(title, message, type, imageUrl) {
...
<!-- 非scoped样式：el-dialog 内容（append-to-body 移到 body）、v-html 内容、Element UI 内部元素 -->
<style>
/* ============ 输入框内部样式（Element UI 内部元素） ============ */
